...
final class ShadowImageView: UIView {
    ...
    private lazy var baseVew: UIView = {
       let v = UIView()
        v.translatesAutoresizingMaskIntoConstraints = false
    ...
    func setupView(){
        addSubview(baseVew)
        baseVew.addSubview(imageView)
        setupConstraints()
    }
    func setupConstraints(){
        [baseVew, imageView].forEach { (v) in
    ...
    override func layoutSubviews() {
        ...
        baseVew.layer.shadowPath = UIBezierPath(roundedRect: baseVew.bounds, cornerRadius: 10).cgPath
        baseVew.layer.shouldRasterize = true
        baseVew.layer.rasterizationScale = UIScreen.main.scale
    }
